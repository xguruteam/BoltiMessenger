...
import org.bolti.SQLite.SQLiteCursor;
import org.bolti.SQLite.SQLiteDatabase;
import org.bolti.SQLite.SQLitePreparedStatement;
import org.bolti.messenger.support.SparseLongArray;
import org.bolti.tgnet.ConnectionsManager;
import org.bolti.tgnet.NativeByteBuffer;
...
public class MessagesStorage {
    ...
    public void openDatabase(boolean first) {
        ...
        try {
            ...
            if (createTable) {
                ...
                database.executeFast("CREATE TABLE wallpapers(uid INTEGER PRIMARY KEY, data INTEGER)").stepThis().dispose();
    ...
    public void putWallpapers(final ArrayList<Integer> wallPapers) {
        storageQueue.postRunnable(() -> {
            try {
                int num = 0;
                ...
                for (Integer wallPaper : wallPapers) {
                    state.requery();
                    state.bindInteger(1, num);
                    state.bindInteger(2, wallPaper.intValue());
                    state.step();
                    num++;
                }
                state.dispose();
    ...
    public void getWallpapers() {
        storageQueue.postRunnable(() -> {
            try {
                ...
                final ArrayList<Integer> wallPapers = new ArrayList<>();
                while (cursor.next()) {
                    wallPapers.add(Integer.valueOf(cursor.intValue(0)));
                }
                cursor.dispose();
