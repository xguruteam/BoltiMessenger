...
package org.bolti.ui.Components;
...
import org.bolti.messenger.ChatObject;
import org.bolti.messenger.DataQuery;
import org.bolti.messenger.FileLoader;
import org.bolti.messenger.ImageReceiver;
import org.bolti.messenger.MessagesController;
import org.bolti.messenger.ContactsController;
...
import org.bolti.messenger.VideoEditedInfo;
import org.bolti.messenger.camera.*;
...
import org.bolti.ui.ActionBar.AlertDialog;
import org.bolti.ui.ActionBar.BaseFragment;
import org.bolti.ui.ActionBar.BottomSheet;
import org.bolti.ui.ActionBar.Theme;
import org.bolti.ui.Cells.PhotoAttachCameraCell;
import org.bolti.ui.Cells.PhotoAttachPhotoCell;
import org.bolti.ui.Cells.ShadowSectionCell;
import org.bolti.ui.ChatActivity;
import org.bolti.ui.PhotoViewer;
...
public class ChatAttachAlert extends BottomSheet implements NotificationCenter.NotificationCenterDelegate, BottomSheet.BottomSheetDelegateInterface {
    ...
    private AttachSendButton sendPhotosButton;
    ...
    private class AttachSendButton extends FrameLayout {
        public TextView textView;
        public TextView textViewCount;
        public FrameLayout linearLayout;
        public ImageView imageView;

        public AttachSendButton(Context context) {
            super(context);

            setBackgroundDrawable(new ColorDrawable(0xff22b83e));
            setPadding(0,0,0,AndroidUtilities.dp(10));

            linearLayout = new FrameLayout(context);

            textViewCount = new TextView(context);
            textViewCount.setLines(1);
            textViewCount.setSingleLine(true);
            textViewCount.setGravity(Gravity.CENTER_HORIZONTAL | Gravity.CENTER_VERTICAL);
            textViewCount.setEllipsize(TextUtils.TruncateAt.END);
            textViewCount.setTextColor(Theme.getColor(Theme.key_dialogInputFieldActivated));
            textViewCount.setTextSize(TypedValue.COMPLEX_UNIT_DIP, 16);
            textViewCount.setText("2");
//            textViewCount.setPadding(AndroidUtilities.dp(5), AndroidUtilities.dp(5), AndroidUtilities.dp(5), AndroidUtilities.dp(5));
            textViewCount.setBackgroundResource(R.drawable.share_round);
            linearLayout.addView(textViewCount, LayoutHelper.createFrame(LayoutHelper.WRAP_CONTENT, LayoutHelper.WRAP_CONTENT, Gravity.CENTER_VERTICAL, 0, 0, 0, 0));
            ...
            linearLayout.addView(textView, LayoutHelper.createFrame(LayoutHelper.WRAP_CONTENT, LayoutHelper.WRAP_CONTENT, Gravity.CENTER_VERTICAL, AndroidUtilities.dp(15), 0, 0, 0));

            imageView = new ImageView(context);
            imageView.setImageResource(R.drawable.attach_close);
            linearLayout.addView(imageView, LayoutHelper.createFrame(LayoutHelper.WRAP_CONTENT, LayoutHelper.WRAP_CONTENT, Gravity.CENTER_VERTICAL, 0, AndroidUtilities.dp(5), 0, AndroidUtilities.dp(5)));

            addView(linearLayout, LayoutHelper.createFrame(LayoutHelper.WRAP_CONTENT, LayoutHelper.MATCH_PARENT, Gravity.CENTER_HORIZONTAL | Gravity.CENTER_VERTICAL, 0, 0, 0, 0));
        }

        @Override
        protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
            super.onMeasure(MeasureSpec.makeMeasureSpec(widthMeasureSpec, MeasureSpec.EXACTLY), MeasureSpec.makeMeasureSpec(AndroidUtilities.dp(90), MeasureSpec.EXACTLY));
        }
    ...
    public ChatAttachAlert(Context context, final BaseFragment parentFragment) {
        ...
        attachView = new FrameLayout(context) {
            ...
            protected void onLayout(boolean changed, int left, int top, int right, int bottom) {
                ...
                int diff = (width - AndroidUtilities.dp(75 * 5 + 20)) / 4;
                ...
                for (int a = 0; a < 7; a++) {
                    ...
                    y = AndroidUtilities.dp(10 + 95 * (num / 5));
                    x = AndroidUtilities.dp(5) + (num % 5) * (AndroidUtilities.dp(75) + diff);
                    views[a].layout(x, y, x + views[a].getMeasuredWidth(), y + views[a].getMeasuredHeight());
                    num++;
                }

                attachPhotoRecyclerView.layout(0, t + AndroidUtilities.dp(105), width, t  + AndroidUtilities.dp(135) + attachPhotoRecyclerView.getMeasuredHeight());
...
//                lineView.layout(0, AndroidUtilities.dp(96), width, AndroidUtilities.dp(96) + lineView.getMeasuredHeight());
                ...
                x = (width - mediaBanTooltip.getMeasuredWidth()) / 2;
                y = t + (attachPhotoRecyclerView.getMeasuredHeight() - mediaBanTooltip.getMeasuredHeight()) / 2;
                mediaBanTooltip.layout(x, y, x + mediaBanTooltip.getMeasuredWidth(), y + mediaBanTooltip.getMeasuredHeight());

                sendPhotosButton.layout(0, AndroidUtilities.dp(239), right, bottom);
            }
        };
        ...
        for (int a = 1; a < 8; a++) {
            ...
            attachView.addView(attachButton, LayoutHelper.createFrame(75, 90, Gravity.LEFT | Gravity.TOP));
        ...
        AttachSendButton sendButton = new AttachSendButton(context);
        attachView.addView(sendButton, LayoutHelper.createFrame(LayoutHelper.MATCH_PARENT, AndroidUtilities.dp(55), Gravity.CENTER_VERTICAL));
        sendPhotosButton = sendButton;
//        sendPhotosButton.imageView.setPadding(0, AndroidUtilities.dp(4), 0, 0);
        sendPhotosButton.setOnClickListener(v -> {
            if (buttonPressed) {
                return;
            }
            buttonPressed = true;
        ...
        attachView.addView(attachPhotoRecyclerView, LayoutHelper.createFrame(LayoutHelper.MATCH_PARENT, PhotoAttachPhotoCell.CELL_SIZE));
        ...
        attachView.addView(progressView, LayoutHelper.createFrame(LayoutHelper.MATCH_PARENT, PhotoAttachPhotoCell.CELL_SIZE));
        ...
        container.addView(cameraPhotoRecyclerView, LayoutHelper.createFrame(LayoutHelper.MATCH_PARENT, PhotoAttachPhotoCell.CELL_SIZE));
    ...
    protected boolean onCustomMeasure(View view, int width, int height) {
        ...
        if (view == cameraView) {
            ...
            if (isPortrait) {
                cameraPhotoRecyclerView.measure(View.MeasureSpec.makeMeasureSpec(width, View.MeasureSpec.EXACTLY), View.MeasureSpec.makeMeasureSpec(AndroidUtilities.dp(PhotoAttachPhotoCell.CELL_SIZE), View.MeasureSpec.EXACTLY));
                ...
                cameraPhotoRecyclerView.measure(View.MeasureSpec.makeMeasureSpec(AndroidUtilities.dp(PhotoAttachPhotoCell.CELL_SIZE), View.MeasureSpec.EXACTLY), View.MeasureSpec.makeMeasureSpec(height, View.MeasureSpec.EXACTLY));
    ...
    private void openCamera(boolean animated) {
        ...
        animateCameraValues[1] = AndroidUtilities.dp(PhotoAttachPhotoCell.CELL_SIZE) - cameraViewOffsetX;
        animateCameraValues[2] = AndroidUtilities.dp(PhotoAttachPhotoCell.CELL_SIZE) - cameraViewOffsetY;
        if (animated) {
        ...
        for (int a = 0; a < count; a++) {
            ...
            if (child instanceof PhotoAttachCameraCell) {
                ...
                if (cameraViewLocation[0] < listViewX) {
                    ...
                    if (cameraViewOffsetX >= AndroidUtilities.dp(PhotoAttachPhotoCell.CELL_SIZE)) {
                        cameraViewOffsetX = 0;
                ...
                if (Build.VERSION.SDK_INT >= 21 && cameraViewLocation[1] < AndroidUtilities.statusBarHeight) {
                    ...
                    if (cameraViewOffsetY >= AndroidUtilities.dp(PhotoAttachPhotoCell.CELL_SIZE)) {
                        cameraViewOffsetY = 0;
                        cameraViewLocation[0] = AndroidUtilities.dp(-150);
                        cameraViewLocation[1] = 0;
                    } else {
    ...
    private void applyCameraViewPosition() {
        if (cameraView != null) {
            ...
            cameraIcon.setTranslationX(cameraViewLocation[0]);
            cameraIcon.setTranslationY(cameraViewLocation[1]);
            int finalWidth = AndroidUtilities.dp(PhotoAttachPhotoCell.CELL_SIZE) - cameraViewOffsetX;
            int finalHeight = AndroidUtilities.dp(PhotoAttachPhotoCell.CELL_SIZE) - cameraViewOffsetY;
    ...
    public void showCamera() {
        ...
        if (cameraView == null) {
            ...
            container.addView(cameraView, 1, LayoutHelper.createFrame(PhotoAttachPhotoCell.CELL_SIZE, PhotoAttachPhotoCell.CELL_SIZE));
            ...
            if (cameraIcon == null) {
                ...
                cameraImageView.setColorFilter(new PorterDuffColorFilter(Theme.getColor(Theme.key_dialogCameraIcon), PorterDuff.Mode.MULTIPLY));
                cameraIcon.addView(cameraImageView, LayoutHelper.createFrame(PhotoAttachPhotoCell.CELL_SIZE, PhotoAttachPhotoCell.CELL_SIZE, Gravity.RIGHT | Gravity.BOTTOM));
            }
            container.addView(cameraIcon, 2, LayoutHelper.createFrame(PhotoAttachPhotoCell.CELL_SIZE, PhotoAttachPhotoCell.CELL_SIZE));
...
//            sendPhotosButton.textView.setText("");
            sendPhotosButton.textViewCount.setVisibility(View.GONE);
            sendPhotosButton.textView.setVisibility(View.GONE);
            sendPhotosButton.imageView.setVisibility(View.VISIBLE);
            sendPhotosButton.setBackgroundDrawable(new ColorDrawable(0xff999999));
            if (baseFragment instanceof ChatActivity) {
...
//            sendPhotosButton.imageView.setBackgroundDrawable(Theme.chat_attachButtonDrawables[8]);
            sendPhotosButton.imageView.setVisibility(View.GONE);
            sendPhotosButton.textViewCount.setVisibility(View.VISIBLE);
            sendPhotosButton.textView.setVisibility(View.VISIBLE);
            sendPhotosButton.setBackgroundDrawable(new ColorDrawable(0xff22b83e));
            if (baseFragment instanceof ChatActivity) {
//                sendPhotosButton.textView.setText(LocaleController.formatString("SendItems", R.string.SendItems, String.format("(%d)", count)));
                sendPhotosButton.textView.setText("Send");
                sendPhotosButton.textViewCount.setText(String.format("%d", count));
...
//                sendPhotosButton.textView.setText(LocaleController.formatString("UploadItems", R.string.UploadItems, String.format("(%d)", count)));
                sendPhotosButton.textView.setText("Upload");
                sendPhotosButton.textViewCount.setText(String.format("%d", count));
            }
        }
            ...
            switch (viewType) {
                ...
                default:
                    FrameLayout frameLayout = new FrameLayout(mContext) {
                        @Override
                        protected void onLayout(boolean changed, int left, int top, int right, int bottom) {
                            int diff = (right - left - AndroidUtilities.dp((PhotoAttachPhotoCell.CELL_SIZE + 5) *3 + 20)) / 2;
                            for (int a = 0; a < 4; a++) {
                                int x = AndroidUtilities.dp(10) + (a % 4) * (AndroidUtilities.dp(PhotoAttachPhotoCell.CELL_SIZE + 5) + diff);
                    ...
                    view = frameLayout;
                    frameLayout.setLayoutParams(new RecyclerView.LayoutParams(LayoutHelper.MATCH_PARENT, AndroidUtilities.dp(PhotoAttachPhotoCell.CELL_SIZE +  20)));
                    break;
            }
